keys = {
    4: "a", 5: "b", 6: "c", 7: "d", 8: "e", 9: "f", 10: "g", 11: "h", 12: "i", 13: "j",
    14: "k", 15: "l", 16: "m", 17: "n", 18: "o", 19: "p", 20: "q", 21: "r", 22: "s", 23: "t",
    24: "u", 25: "v", 26: "w", 27: "x", 28: "y", 29: "z", 30: "1", 31: "2", 32: "3", 33: "4",
    34: "5", 35: "6", 36: "7", 37: "8", 38: "9", 39: "0", 40: "\n", 44: " ", 45: "-", 46: "=",
    47: "[", 48: "]", 49: "\", 51: ";", 52: "\x27", 53: "`", 54: ",", 55: ".", 56: "/"
}
import subprocess
cmd = "tshark -r soal15_wired_usb_hid.pcap -Y "usb.capdata" -T fields -e usb.capdata"
output = subprocess.check_output(cmd, shell=True).decode().splitlines()
result = []
for line in output:
    if len(line) >= 4:
        # Ambil byte ke-3 (indeks 4 dan 5) sebagai keycode utama
        try:
            code = int(line[4:6], 16)
            if code in keys:
                result.append(keys[code])
        except:
            pass
print("".join(result))